...
config["Port"] = 4901 # Unique ID, can be any number but must match client's
...
def update_state(ID,clients, startTime):
    ...
    while (True):
        ...
        # for client in clients:
        #     client.send(pickle.dumps([current_state, t-startTime]))
...
def threaded_client(clients, ID, temp_game_data, startTime):
    ...
    while True:
        ...
        if data == None: 
            ...
            print('data to send')
            print(data)
            clients[not ID].send(pickle.dumps([data, time.time()-startTime]))
